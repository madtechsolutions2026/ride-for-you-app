...
export const FleetLiveMap: React.FC = () => {
  ...
  // Plot Clean Neumorphic Markers with #1F6F43
  useEffect(() => {
        ...
        const icon = L.divIcon({
          ...
          html: `
            <div style="
              ...
              border: ${isSelected ? '2px solid #1F6F43' : '1px solid #E5E2DB'};
              cursor: pointer;
              white-space: nowrap;
            ...
            ">
              ...
              <div style="display: flex; flex-direction: column; line-height: 1.1;">
                <span style="font-size: 11px; font-weight: 800; color: #16150F;">${hub.name.split(' ')[0]} Hub</span>
                <span style="font-size: 9.5px; font-weight: 700; color: #1F6F43;">${hub.availableBikes} Bikes Ready</span>
              </div>
            </div>
          `,
          iconSize: [0, 0],
        ...
        const icon = L.divIcon({
          ...
          html: `
            <div style="
              ...
              border: ${isSelected ? '2px solid #1F4E79' : '1px solid #E5E2DB'};
              cursor: pointer;
              white-space: nowrap;
              font-family: 'Poppins', sans-serif;
              transform: translate(-50%, -50%);
            ...
            ">
              ...
              ">⚡</div>
              <div style="display: flex; flex-direction: column; line-height: 1.1;">
                <span style="font-size: 11px; font-weight: 800; color: #16150F;">${station.name.split(' ')[0]}</span>
                <span style="font-size: 9.5px; font-weight: 700; color: #1F4E79;">${station.chargedBatteries}/${station.totalDocks} Swaps</span>
              </div>
            </div>
          `,
          iconSize: [0, 0],
    ...
    vehicles.forEach((bike) => {
      ...
      const dotColor = isLow ? '#A02724' : isMoving ? '#1F6F43' : '#16150F';
      const badgeBg = isLow ? '#FBEDEC' : '#EDF3EF';
      const badgeColor = isLow ? '#A02724' : '#1F6F43';
      ...
      const icon = L.divIcon({
        ...
        html: `
          ...
          ">
            ...
            "></span>
            <span style="font-size: 11px; font-weight: 800; color: #16150F; font-family: monospace;">${bike.plate}</span>
            <span style="
              font-size: 10px;
              font-weight: 800;
              background: ${badgeBg};
  ...
  return (
    <div className="bg-white rounded-3xl border border-[#E5E2DB] shadow-neo overflow-hidden relative font-sans">
      {/* Top Bar with #1F6F43 Neumorphism */}
      <div className="p-4 sm:p-5 border-b border-[#E5E2DB] bg-white flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h3 className="text-base font-extrabold text-[#16150F] tracking-tight flex items-center gap-2">
        ...
        <div className="flex flex-wrap items-center gap-1.5 p-1 bg-[#F4F2ED] rounded-2xl border border-[#E5E2DB] shadow-neo-sm">
          <button
          ...
          >
            <span className="w-2 h-2 rounded-full bg-[#1F6F43]"></span>
            <span>Moving (14)</span>
          </button>
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-extrabold transition-all duration-150 flex items-center gap-1 ${
              activeFilter === 'LOW_BATTERY'
                ? 'bg-[#A02724] text-white shadow-sm'
                : 'text-[#A02724] hover:bg-[#FBEDEC]'
            }`}
          >
      ...
      <div className="relative w-full h-[520px]">
        ...
        <button
          ...
          className="absolute top-4 right-4 z-10 p-3 bg-white/95 backdrop-blur-md rounded-2xl border border-[#E5E2DB] shadow-neo text-[#16150F] hover:text-[#1F6F43] transition"
        ...
        <div className="absolute bottom-4 left-4 z-10 hidden sm:flex items-center gap-3 bg-white/95 backdrop-blur-md px-4 py-2.5 rounded-2xl border border-[#E5E2DB] shadow-neo text-xs font-extrabold text-[#16150F]">
          <div className="flex items-center gap-1.5">
            <Battery className="w-4 h-4 text-[#1F6F43]" />
            <span>Avg Fleet SoC: <strong className="text-[#1F6F43]">78%</strong></span>
          </div>
          <span className="text-[#D6D2C8]">•</span>
          <div className="flex items-center gap-1.5">
          ...
          <span className="text-[#D6D2C8]">•</span>
          <div className="flex items-center gap-1.5">
            <Shield className="w-4 h-4 text-[#1F6F43]" />
        ...
        {selectedAsset && (
          <div className="absolute top-4 left-4 right-4 sm:left-auto sm:right-4 sm:w-96 bg-white/95 backdrop-blur-md rounded-3xl border border-[#E5E2DB] shadow-neo p-5 z-20 animate-in slide-in-from-top-4 duration-200">
            {/* Header */}
            <div className="flex items-center justify-between pb-3 border-b border-[#E5E2DB]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-sm font-extrabold text-[#16150F]">
                    {selectedAsset.plate || selectedAsset.name}
                  </h4>
                  <p className="text-xs font-medium text-[#7A756B]">
              ...
              <button
                ...
                className="w-7 h-7 rounded-full bg-[#F4F2ED] text-[#7A756B] hover:text-[#16150F] flex items-center justify-center text-xs font-bold shadow-neo-sm"
              >
                ✕
              </button>
            </div>
            ...
            {/* Vehicle Details */}
            {selectedAsset.type === 'BIKE' && (
              <div className="mt-3.5 space-y-3 text-xs font-semibold text-[#16150F]">
                <div className="grid grid-cols-2 gap-2.5">
                  <div className="p-3 bg-[#F4F2ED] rounded-2xl border border-[#E5E2DB] shadow-neo-sm">
                    <span className="text-[10px] text-[#7A756B] uppercase font-bold">IoT State</span>
                    <p className="text-xs font-extrabold mt-0.5 flex items-center gap-1.5">
                      <span
                        className={`w-2 h-2 rounded-full ${
                          selectedAsset.status === 'MOVING' ? 'bg-[#1F6F43] animate-ping' : 'bg-[#16150F]'
                        }`}
                      />
                  ...
                  <div className="p-3 bg-[#F4F2ED] rounded-2xl border border-[#E5E2DB] shadow-neo-sm">
                    <span className="text-[10px] text-[#7A756B] uppercase font-bold">Battery SoC</span>
                    <p
                      className={`text-xs font-extrabold mt-0.5 ${
                        selectedAsset.battery < 20 ? 'text-[#A02724]' : 'text-[#1F6F43]'
                      }`}
                    >
                ...
                <div className="p-3 bg-[#F4F2ED] rounded-2xl border border-[#E5E2DB] space-y-1.5 text-[11px] shadow-neo-sm">
                  <div className="flex justify-between text-[#7A756B]">
                    <span>Rider:</span>
                    <strong className="text-[#16150F]">{selectedAsset.rider}</strong>
                  </div>
                  <div className="flex justify-between text-[#7A756B]">
                    <span>Assigned Hub:</span>
                    <strong className="text-[#16150F]">{selectedAsset.hub}</strong>
                  </div>
                  <div className="flex justify-between text-[#7A756B]">
                    <span>Voltage / Temp:</span>
                    <strong className="text-[#16150F]">{selectedAsset.voltage} • {selectedAsset.temp}</strong>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <button
                    ...
                    className="py-2.5 px-3 rounded-2xl bg-[#FBEDEC] text-[#A02724] font-bold text-xs hover:bg-[#EFD2D0]/30 transition flex items-center justify-center gap-1.5 shadow-neo-sm"
                  >
                  ...
                  <button
                    ...
                    className="py-2.5 px-3 rounded-2xl bg-[#EDF3EF] text-[#1F6F43] font-bold text-xs hover:bg-[#D3E4DA] transition flex items-center justify-center gap-1.5 shadow-neo-sm"
                  >
            ...
            {selectedAsset.type === 'HUB' && (
              <div className="mt-3.5 space-y-3 text-xs font-semibold text-[#16150F]">
                <div className="p-3 bg-[#EDF3EF] rounded-2xl border border-[#D3E4DA] text-center shadow-neo-sm">
                  <span className="text-[10px] text-[#1F6F43] uppercase font-bold">Available Fleet</span>
                  <h4 className="text-lg font-extrabold text-[#1F6F43]">
                    {selectedAsset.availableBikes} / {selectedAsset.totalBikes} Bikes Ready
                  </h4>
                </div>
                <div className="text-[11px] text-[#7A756B] space-y-1">
            ...
            {selectedAsset.type === 'SWAP' && (
              <div className="mt-3.5 space-y-3 text-xs font-semibold text-[#16150F]">
                <div className="p-3 bg-[#EDF1F6] rounded-2xl border border-[#D8E1EB] text-center shadow-neo-sm">
                  <span className="text-[10px] text-[#1F4E79] uppercase font-bold">Fast Swaps</span>
                  <h4 className="text-lg font-extrabold text-[#1F4E79]">
                    {selectedAsset.chargedBatteries} / {selectedAsset.totalDocks} Batteries Ready
                  </h4>
                </div>
                <p className="text-[11px] text-[#7A756B]">~90 Second Automatic Swap Cycle Active</p>
              </div>
            )}

            {commandFeedback && (
              <div className="mt-3 p-2.5 rounded-2xl bg-[#16150F] text-white text-xs font-bold text-center animate-in fade-in">
                {commandFeedback}
              </div>
            )}
          </div>
